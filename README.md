...
**An H.265 encoder** needs one setting: `ENCODER_CODEC=h265`. The player will not switch its
video decoder mid-recording — the same limit that keeps the pre-roll and program on one
stream — so the black played inside the hand-off has to be the program's own codec, or the
picture freezes on it instead of crossing to the program. This image's ffmpeg has no
software H.265 encoder, so ah4c cannot make an H.265 black at startup; a half-second clip is
built once, offline, and shipped inside the binary, and `ENCODER_CODEC=h265` selects it. The
pre-roll itself is left as it is — an ordinary H.264 pre-roll is fine, there is nothing to
convert or re-export — and the H.265 black is placed as the seam in front of the program so
its H.265 decoder has a clean same-codec frame to start on. Left at its `h264` default,
nothing changes.
...
services:
  ...
  ah4c:
    ...
    environment:
      ...
      - ENCODER_CODEC=${ENCODER_CODEC:-h264} # The video codec your encoder outputs: h264 (default) or h265. The black played inside a PLAYBACK_DELAY / PLAYBACK_DETECTION hand-off, and the seam placed in front of the program when a pre-roll is used, must be that same codec — a player will not switch its video decoder across the hand-off, so a filler in the wrong codec freezes on itself instead of crossing to the program. This image has no software H.265 encoder, so the H.265 black is a clip shipped in the binary; H.264 is generated at startup as before. With h265 the pre-roll is left as it is (an ordinary H.264 pre-roll is fine) and the H.265 black is inserted as the seam. Leave at h264 unless your encoder is set to H.265. Case-insensitive; h265 and hevc both work.
